Golang script that uses the Gmail API to authenticate via OAuth 2.0, fetch all emails from a specified Gmail account (including sent and received), and export them to a structured JSON file. The script should handle pagination, rate limits, and extract key metadata (subject, sender, recipient, date, body text).
